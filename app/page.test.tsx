...
describe('Home', () => {
  it('shows the form view with correct title initially', () => {
    render(<Home />)
    expect(screen.getByText('顔登録用のQR作成')).toBeInTheDocument()
  ...
  it('switches to QR view when form generates a payload', async () => {
    ...
    expect(screen.queryByText('顔登録用のQR作成')).not.toBeInTheDocument()
  })
  ...
  it('switches to personnel form via hamburger menu', async () => {
    ...
    await user.click(screen.getByText('入場用のQR作成'))
    expect(screen.getByText('入場用のQR作成')).toBeInTheDocument()
  ...
  it('switches back to visitor form via hamburger menu', async () => {
    ...
    await user.click(screen.getByText('入場用のQR作成'))
    await user.click(screen.getByRole('button', { name: 'メニュー' }))
    await user.click(screen.getByText('顔登録用のQR作成'))
    expect(screen.getByText('顔登録用のQR作成')).toBeInTheDocument()
